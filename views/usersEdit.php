...
if ($_SESSION['dataUser']['role'] !== 'Peserta'):
    ...
    // get id from URL
    $id = $_GET['id'];
    // make object from class UserModel
    $userModel = new UserModel();
    // get all records from contactModel
    $contactRecords = $contactModel->findAll();
    // get 1 record from userModel
    $userRecord = $userModel->find($id);
    ?>
            ...
            <div class="col-sm-6">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <label for="id_users" class="form-label">Unique ID User</label>
                        <input type="text" class="form-control mb-3" name="id_users" value="<?= $userRecord['id_users'] ?>" readonly disabled>
                            ...
                            <input type="text" class="form-control mb-3" id="username" name="username"
                                ...
                                value="<?= $userRecord['username'] ?>" readonly disabled>
                            <label for="role" class="form-label">Role</label>
                            <select name="role" id="role" class="form-control mb-3">
                                <option value="">-- Pilih Role --</option>
                                <option value="Mentor" <?= ($userRecord['role'] == 'Mentor' ? 'selected' : '') ?>>Mentor
                                </option>
                                <option value="Peserta" <?= ($userRecord['role'] == 'Peserta' ? 'selected' : '') ?>>Peserta
                                </option>
                            </select>
                            ...
                            <select name="id_person" id="id_person" class="form-control mb-3"
                                ...
                                <?php foreach ($contactRecords as $key => $value): ?>
                                    ...
                                    <?= ($userRecord['id'] == $value['id'] ? 'selected' : '') ?>
                                    >
                                        <?= $value['nama_lengkap'] ?>
                                    </option>
                                <?php endforeach; ?>
                            ...
                            <input type="hidden" name="id_users" value="<?= $userRecord['id_users'] ?>">
